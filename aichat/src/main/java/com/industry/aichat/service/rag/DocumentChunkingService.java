package com.industry.aichat.service.rag;
...
public class DocumentChunkingService {
    ...
    public List<DocumentChunk> chunkDocument(ChatFile file, String content) {
        ...
        for (String paragraph : paragraphs) {
            if (paragraph.length() > CHUNK_SIZE) {
                ...
                // 其他段落：合并到finalChunks，后续统一处理
                finalChunks.add(paragraph);
            }
        }

        // 策略3：合并过小的段落，确保每个chunk至少达到MIN_CHUNK_SIZE
        List<String> mergedChunks = mergeSmallChunks(finalChunks, MIN_CHUNK_SIZE);

        // 创建DocumentChunk对象
        int currentPosition = 0;
        for (int i = 0; i < mergedChunks.size(); i++) {
            String chunkContent = mergedChunks.get(i);
    ...
    /**
     * 合并过小的段落，确保每个chunk至少达到minSize
     *
     * 将相邻的小段落合并为一个chunk，如果合并后仍不足minSize但已经是最后一段，也保留。
     *
     * @param chunks  段落列表
    ...
    private List<String> mergeSmallChunks(List<String> chunks, int minSize) {
        ...
        for (String chunk : chunks) {
            if (buffer.length() + chunk.length() > 0) {
                buffer.append("\n\n");
            }
            buffer.append(chunk);
        ...
        // 剩余内容也作为最后一个chunk（即使不足minSize，内容不应丢弃）
        if (buffer.length() > 0) {
            merged.add(buffer.toString().trim());
        }
